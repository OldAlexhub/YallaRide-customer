...
const TripInProgressScreen = () => {
  ...
  const [driverToDropoffRoute, setDriverToDropoffRoute] = useState([]);
  ...
  useEffect(() => {
    const fetchTrip = async () => {
      try {
        ...
        if (driverLocation && dropoff) {
          const route = await getDirections(driverLocation, dropoff);
          setDriverToDropoffRoute(route);
        }
      } catch (error) {
    ...
    <View style={styles.container}>
      {region && (
        <MapView style={styles.map} initialRegion={region}>
          ...
          {driverToDropoffRoute.length > 0 && (
            <Polyline coordinates={driverToDropoffRoute} strokeColor="#22c55e" strokeWidth={4} />
          )}
        </MapView>
      )}
      ...
      <BottomSheet>
        ...
        <Text style={styles.text}>Sit back and relax. We&apos;ll get you there.</Text>
